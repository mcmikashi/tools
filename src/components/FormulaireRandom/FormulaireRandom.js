...
function FormulaireRandom(){
  ...
  <li ref={i => listeElements.push(i)} key={index} className="list-group-item d-flex justify-content-between align-items-center bgAnimationRed">
    {element}<button className="ms-1 btn btn-sm btn-outline-danger" onClick={(e) => {effacerElementClick(e, index);}}><i class="bi bi-trash"></i></button>
  </li>)
  ...
  async function animationAffichage() {
    ...
    listeElements[i].className = "list-group-item d-flex justify-content-between align-items-center bg-danger"
    console.log(listeElements)
    if (i!==0) {
      listeElements[i-1].className  = "list-group-item d-flex justify-content-between align-items-center"
    }
    await new Promise(r => setTimeout(r, 250));
    }
    listeElements[arr.length-1].className  = "list-group-item d-flex justify-content-between align-items-center" 
}	
async function ChoixRandom() {
    ...
    if (checkBoxEquipe.current.checked===true) {
        ...
        for (let i = 0; i < lastArray.length; i++) {
          ...
          arrayAffichageRandom.push(createElement('p',{key:i},'Equipe '+(i+1)+' :'))
          let liListe = []
          for (const element of arrayIn) {
              liListe.push(createElement('li',{key:element,className:"list-group-item"},element))
          }
          let listeEquipe = createElement('ul',{className:'list-group',key:i},liListe)
      ...
      var elementsSpan = arr.map((element,index) => {
        let listeLiRandom = [];
        if (index===bonIndex) {
          listeLiRandom.push(<li key={index} className="list-group-item active">{element}</li>)
        }else{
          listeLiRandom.push(<li key={index} className="list-group-item">{element}</li>) 
        }
        return listeLiRandom;
      })
      /* creation conteneur liste */
      setAffcihage(createElement('ul',{className:"list-group"},elementsSpan))
      } 
    }
  return(
  <div className="row gap-4" data-testid="FormulaireRandom">
      <div className="col">
        <div className="p-3 border bg-light">
            {/* /formulaire/ */}
            <div className="mb-2">
              <label htmlFor="textareaListeNom" className="form-label">Ajout multiple (séparer par une virgule) :</label>
              <textarea ref={textareaListeNom} className="form-control" id="textareaListeNom" rows="4"></textarea>
            </div>
            <div className="mb-2">
              <label htmlFor="radioEquipes" className="form-label">Equipe :</label>
              <div>
                <div className="form-check form-check-inline">
                  <div className="form-check form-switch">
                    <input ref={checkBoxEquipe}  onChange={changementEquipe} className="form-check-input" type="checkbox" id="checkBoxEquipe"/>
                  </div>
                </div>
              </div>
            </div>
            <div className="mb-2">
              <label htmlFor="inputNombreEquipe" className="form-label">Nombre de personnes par equipe :</label>
              <input ref={inputNombreEquipe} className="form-control" type="number" name="inputNombreEquipe" id="inputNombreEquipe" min="2" defaultValue="2" disabled />
            </div>
            <div className="d-grid gap-2 col-6 mx-auto">
                <button ref={btnAjouter} className="btn btn-outline-primary" onClick={ajouterClick}>Ajouter</button>
                <button ref={btnEffacer} className="btn btn-outline-danger" onClick={effacerClick}>Effacer</button>
                <button ref={btnLancer} className="btn btn-outline-success" onClick={ChoixRandom}>Lancer</button>
            </div>
        </div>
      </div>
      <div className="col">
      ...
      <div className="col">
            <div ref={divAffichage} className="p-3 border bg-light">
              {affcihage}
            </div>
      </div>
  </div>
